...
public class TwoMinsTipPlugin implements InstructPluginInterface{
	/**
	 * 测试用，消息 拼出来的
	 * @param user
	 * @param orgi
	...
	public String getMessage(Instruction instruct ,AgentUser user , String orgi , Channel channel){
		...
		SearchResultTemplet srt=(SearchResultTemplet) (EapDataContext.getService().findAllByCriteria(DetachedCriteria.forClass(SearchResultTemplet.class).add(Restrictions.eq("code", getCode()))).get(0));
	...
	public String getCode() {
		...
		return EapSmcDataContext.TemplateCodeEnum.TWOMINSTIP.toString();
	}
	@Override
